...
from app.db.models import Checkin, Goal, Habit
...
def get_overview(user_id: int = Depends(get_user_id), db: Session = Depends(get_db)) -> OverviewOut:
    today = dt.date.today()
    since = today - dt.timedelta(days=6)

    habits_ids = list(db.scalars(select(Habit.id).where(Habit.user_id == user_id)))
    habits_count = len(habits_ids)
    goals_count = int(db.scalar(select(func.count(Goal.id)).where(Goal.user_id == user_id)) or 0)
    checkins_7d = int(
        db.scalar(
            select(func.count(Checkin.id)).where(Checkin.user_id == user_id, Checkin.date >= since)
        )
        or 0
    )
    ...
    for hid in habits_ids:
        try:
            streak_total += get_streak(db=db, user_id=user_id, habit_id=hid)
        except Exception:
            pass

    try:
        col = get_diary_collection()
        diary_entries = int(col.count_documents({"user_id": user_id}))
    except Exception:
        diary_entries = 0
